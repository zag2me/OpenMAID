...
function lastupdated($feed_type, $count){
	...
	$rss->link = "http://www.meedios.com/OpenMAID/" . $filename; 
	$rss->syndicationURL = "http://www.meedios.com".$_SERVER["PHP_SELF"]; 

	$image = new FeedImage(); 
	$image->title = "meedios logo"; 
	$image->url = "http://www.meedios.com/OpenMAID/images/meedios_logo.jpg"; 
	...
	while ($data = mysql_fetch_object($res)) { 
	    ...
	    $item->link = "http://www.meedios.com/OpenMAID/detail.php?plugin_id=" . $data->plugin_ID; 
    	...
    	$additionalElements = array("version"=>$data->plugin_Version, 
											...
											"download_link"=>"http://www.meedios.com/OpenMAID/" . $data->plugin_DownloadLink, 
		...
		$sstmp = array();
		$screenshots = array("screenshot_thumb_0"=>"http://www.meedios.com/OpenMAID/images/missing.gif","screenshot_0"=>"http://www.meedios.com/OpenMAID/images/missing.gif");
		$nb = mysql_numrows($ss);
		if ($nb > 0)
		{
			...
			while ($i < $nb)
			{
				$image_thumb_path = "http://www.meedios.com/OpenMAID/" . mysql_result($ss, $i, "image_thumb_path");
				$image_path = "http://www.meedios.com/OpenMAID/" . mysql_result($ss, $i, "image_path");
