...
function CalculateVersionValue($version, $ignore_sanity)
{
	...
	if (ereg('[^A-Za-z0-9. ]', $version)) { 
		...
		foreach($versions as $ver) {	
			$verValue .= str_pad($ver,3,"0",STR_PAD_LEFT);
			//$verValue = $verValue / 1000000;
			}
	}
	if ($ignore_sanity) $failed_version_check = 0;
...
function VoteWorking($profil_id, $plugin_id, $plugin_version, $vote, $uniqueID)
{
	...
	if ($vote == "")
	{
		...
		$totalvotes = $yes+$no;
		if ($totalvotes > 0) {
			$percentyes = round(($yes/$totalvotes)*100);
			$percentno = round(($no/$totalvotes)*100);
		}

		//header
		echo "<table border='0' cellpadding='0' cellspacing='0'><tr>";
		if ($totalvotes > 0 && $percentyes > 55) echo "<td align='left' valign='middle' rowspan='2'><img src='images/thumbsup.gif' width='65%' height='65%' border='0'></td>";
		elseif ($totalvotes > 0 && $percentyes < 45) echo "<td align='left' valign='middle' rowspan='2'><img src='images/thumbsdown.gif' width='65%' height='65%' border='0'></td>";
		elseif ($totalvotes < 1) echo "<td align='left' valign='middle' rowspan='2'><img src='images/unknown.gif' width='65%' height='65%' border='0'></td>";
		echo "<td>Does this plugin work?&nbsp;&nbsp;";
		//your vote
		if ($profil_id == "") echo "<a href='http://www.meedios.com/forum/login.php'>Login to vote</a>";
		else
		{
			...
			if ($nb == 0)
			{
				echo "<br>&nbsp;<a href=\"$uniqueID&mode=vote&profile_id=$profil_id&plugin_id=$plugin_id&plugin_version=$plugin_version&vote=true\">YES</a> or <a href=\"$uniqueID&mode=vote&profile_id=$profil_id&plugin_id=$plugin_id&plugin_version=$plugin_version&vote=false\">NO</a>";
			}
			elseif ($nb == 1)
			{
				echo "<br>&nbsp;<a href=\"$uniqueID&mode=vote&profile_id=$profil_id&plugin_id=$plugin_id&plugin_version=$plugin_version&vote=true\">YES</a> or <a href=\"$uniqueID&mode=vote&profile_id=$profil_id&plugin_id=$plugin_id&plugin_version=$plugin_version&vote=false\">NO</a>";
		...
		if ($totalvotes == 0) echo "&nbsp;(Community hasn't voted yet)";
		elseif ($totalvotes > 0 && $percentyes > 55) echo "&nbsp;(Community says Yes)<br> &nbsp;<b>Yes: $percentyes% ($yes)</b> &nbsp;&nbsp;&nbsp;No: $percentno% ($no) ";
		elseif ($totalvotes > 0 && $percentyes < 45) echo "&nbsp;(Community says No)<br> &nbsp;Yes: $percentyes% ($yes) &nbsp;&nbsp;&nbsp;<b>No: $percentno% ($no)</b> ";
		elseif ($totalvotes > 0 && $percentyes <= 55 && $percentyes >= 45) echo "&nbsp;(Community is undecided...be cautious)<br> &nbsp;Yes: $percentyes% ($yes) &nbsp;&nbsp;&nbsp;No: $percentno% ($no) ";
   ...
   $depth[$parser]--;
	...
	switch($name)
	{
		...
		case "PLUGIN-ID":
			...
			//added code to convert id to all uppercase
			$plugin_id = addslashes(strtoupper($data[$name]));
			break;						
		case "PLUGIN-VERSION":
...
function AdminAddMpp($file, $ignore_version, $ignore_sanity, $delete_upload, $force_update, $process_multiple, $u)
{
	...
	$uploaded_file_tmp = explode('/',$file);
	$uploaded_file_cnt = count($uploaded_file_tmp) - 1;
	$uploaded_file = $uploaded_file_tmp[$uploaded_file_cnt];
	if (!$temp_directory) {
		echo "Error creating temp directory! Sorry, we can't continue... <br><br>\n";
		return FALSE;
		}
	
	echo "Processing file: $uploaded_file<br>\n";
	...
	while ($data = @fread($fp, 4096)) {
		if (!@xml_parse($xml_parser, $data, feof($fp))) {
			...
			echo "<b>Error in .plugin file: '$msg' on line $line</b><br>\n";
			$parser_failed = TRUE;
		}
	}
	...
	$thismpp = getPluginFileName($plugin_name, $plugin_version, ucfirst($plugin_state));
	echo "<b>Continuing to process plugin: $thismpp</b><br>";
	...
	if ( ($plugin_type != "theme" && $module_type == "icon") || $plugin_type == "icon") {
		echo "<br><b>ERROR: There is an inconsistency with your plugin and module types!<br>";
		echo "If you are uploading an icon set, please set the PLUGIN-TYPE in the <br>";
		echo ".plugin file to Theme, set the MODULE-TYPE to Icon, and re-upload the set.<br>";
		echo "If you are not uploading an icon set, please check your plugin and module type<br>";
		echo "settings in the .plugin file and try uploading again.</b><br><br>";
		$pluginfile_error = TRUE;
		}
	...
	//added code to put UL By: username in profil_id of plugin record so admin can see who uploaded a plugin
	//if the plugin was uploaded with 'Upload Not as Author' checked on the upload page.
	if (substr($u,0,6) == "UL By:") $u2 = "";
	else $u2 = $u;
	$pid_passed = CheckProfileIDUpload($plugin_id, $plugin_author, $u2);
	...
	$plugin_VersionValue = ltrim(CalculateVersionValue($plugin_version,$ignore_sanity),"0");
	...
	$plugin_DownloadLink = $plugin_home_directory . "/" . $plugin_id . "/" . $plugin_version . "/" . strtolower($plugin_state) . "/" .$thismpp;
		...
		else 
	{
		...
		if ($force_update == TRUE) {
			...
			$sql = "DELETE FROM images WHERE plugin_ID='$plugin_id' and plugin_Version='$plugin_version'";
			@mysql_query($sql); 
			
			$sql  = "UPDATE plugins SET plugin_ID='$plugin_id', plugin_Name='$plugin_name', plugin_ShortDescription='$plugin_description', plugin_Type='$pt', ";
		...
		if ($delete_upload) {
			...
			if (!rename("$file", "$plugin_dest_directory/" . $thismpp))
			{
				@unlink($file);
			...
			if (!copy("$file", "$plugin_dest_directory/" . $thismpp))
			{
		...
		$screenshots = array();
		findFiles($screenshots, $temp_directory . sql_regcase("/screenshot.jpg"));
		findFiles($screenshots, $temp_directory . sql_regcase("/screenshots/*.jpg"));
		findFiles($screenshots, $temp_directory . sql_regcase("/screenshots/*.jpeg"));
		//echo "<br>"; echo "sql_regcase: " . $temp_directory . sql_regcase("/screenshot.jpg") . "<br>screenshots array:"; print_r($screenshots); echo "<br>";
		...
		echo "<b>SUCCESS!  ($thismpp)</b><br>\n";

	//Do some cleanup
	//Delete Temp data directory
	echo "<br><b>Cleanup:</b><br>Clearing Temporary directory...";
	remove_directory($temp_directory);
	echo "Done.<br>";
...
function TestPluginFile($pluginfile, $u)
{
	...
	$plugin_IsDotNET = "";
	$plugin_IsDotNETSimple = "";
	$uploaded_file_tmp = explode('/',$pluginfile);
	$uploaded_file_cnt = count($uploaded_file_tmp) - 1;
	$uploaded_file = $uploaded_file_tmp[$uploaded_file_cnt];
   
	
	echo "Checking .plugin file: $uploaded_file<br>\n";
...
function xtern_summary($gen_cache){
	...
	if ($gen_cache || $send_gen_cache) {
		if (file_exists($summary_cache_tempfile)) unlink($summary_cache_tempfile) or slowDie("Error!  <i>Summary</i> temp cache file is in use, try again later.");
		$filehandle = fopen($summary_cache_tempfile, 'w') or slowDie("Error opening temp cache file: $summary_cache_tempfile");
		...
		$sql = "SELECT plugin_ID, max(plugin_VersionValue) as max_VersionValue, max(plugin_Date) as max_Date FROM plugins WHERE plugin_Type='Module' OR plugin_Type='Theme' OR plugin_Type='Import' OR plugin_Type='Input' OR plugin_Type='General' OR plugin_Type='Icon' OR plugin_Type='Misc' AND plugin_ReviewFlag='FALSE' GROUP BY plugin_ID";
		...
		$res = mysql_query($sql);
			...
			while (($enreg=@mysql_fetch_array($res))) {
				...
				fwrite($filehandle, "url=\"" . $sys_url . "dl.php?plugin=$plugin_id&plugin_version=$plugin_version&plugin_state=$plugin_state\" versionvalue=\"$plugin_versionvalue\" size=\"$plugin_downloadsize\" date=\"$plugin_date\" ");
		...
		//once we have the temp cache file, delete the old cache file and rename the temp cache file
		if (!file_exists($summary_cache_tempfile)) slowDie("Error!  <i>Summary</i> temp cache file wasn't created.");
...
function xtern_extended($gen_cache){
	...
	//once we have the temp cache file, delete the old cache file and rename the temp cache file
	if ($gen_cache || $send_gen_cache) {
		if (file_exists($extended_cache_tempfile)) unlink($extended_cache_tempfile) or slowDie("Error!  <i>Extended</i> temp cache file is in use, try again later.");
		$filehandle = fopen($extended_cache_tempfile, 'w') or slowDie("Error opening temp cache file: $extended_cache_tempfile");
		...
		foreach ($state as $stateValue){
			...
			$sql .= "WHERE (plugin_Type='Module' OR plugin_Type='Theme' OR plugin_Type='Import' OR plugin_Type='Input' OR plugin_Type='General' OR plugin_Type='Icon' OR plugin_Type='Misc') AND plugin_State='$stateValue' AND plugin_ReviewFlag='FALSE' ";
		...
		//once we have the temp cache file, delete the old cache file and rename the temp cache file
		if (!file_exists($extended_cache_tempfile)) slowDie("Error!  <i>Extended</i> temp cache file wasn't created.");
...
function xtern_all($gen_cache){
	...
	//once we have the temp cache file, delete the old cache file and rename the temp cache file
	if ($gen_cache || $send_gen_cache) {
		if (file_exists($all_cache_tempfile)) unlink($all_cache_tempfile) or slowDie("Error!  <i>All</i> temp cache file is in use, try again later.");
		$filehandle = fopen($all_cache_tempfile, 'w') or slowDie("Error opening temp cache file: $all_cache_tempfile");
		...
		//$sql = "SELECT * FROM plugins ORDER BY plugin_Name";
		$sql = "SELECT * FROM plugins WHERE plugin_Type='Module' OR plugin_Type='Theme' OR plugin_Type='Import' OR plugin_Type='Input' OR plugin_Type='General' OR plugin_Type='Icon' OR plugin_Type='Misc' AND plugin_ReviewFlag='FALSE' ORDER BY plugin_Name ASC";
		$res = mysql_query($sql);
		...
		//once we have the temp cache file, delete the old cache file and rename the temp cache file
		if (!file_exists($all_cache_tempfile)) slowDie("Error!  <i>All</i> temp cache file wasn't created.");
...
function xtern_sync($gen_cache){
	...
	//once we have the temp cache file, delete the old cache file and rename the temp cache file
	if ($gen_cache || $send_gen_cache) {
		if (file_exists($sync_cache_tempfile)) unlink($sync_cache_tempfile) or slowDie("Error!  <i>Sync</i> temp cache file is in use, try again later.");
		$filehandle = fopen($sync_cache_tempfile, 'w') or slowDie("Error opening temp cache file: $all_cache_tempfile");
		...
		//once we have the temp cache file, delete the old cache file and rename the temp cache file
		if (!file_exists($sync_cache_tempfile)) slowDie("Error!  <i>Sync</i> temp cache file wasn't created.");
...
function regenerate_xtern_cache() {
			$temp_files = array();
			$temp_files = file_list("/", "*.temp", 1, TRUE);
			$temp_files_count = count($temp_files);
			echo "Regenerating xtern cache files...<br>";
			if ($temp_files_count > 0 )
			{
				foreach ($temp_files as $file) {
					$diff = floor((time() - filemtime("$file"))/60/60/24);
					if ($diff > 0) echo "... $file was $diff days old.<br>";
					delfile($file);
					}
			}
...
//function to take php.ini mem sizes with K, M, or G and convert to bytes.
...
function getPluginFileName($plugin_Name, $plugin_Version, $plugin_State) {
		...
		$plugin_Name = str_replace("<","[",$plugin_Name);
		$plugin_Name = str_replace(">","]",$plugin_Name);
		return $plugin_Name . "_" . $plugin_Version . "_" . $plugin_State . ".mpp";
}
?>