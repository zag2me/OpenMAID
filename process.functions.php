...
function manage_flagged($status, $mode) {
	...
	echo "<tr><th>Name</th><th>Version</th><th>State</th><th>Date</th><th>Author</th><th>Profile ID</th><th>Flag</th><th>Actions</th></tr>";
	...
	while ( $row=mysql_fetch_array($res,MYSQL_NUM)) {
		...
		foreach($row as $field)
		{
			...
			elseif ($field_num == 6) {
				if (substr($field,0,6) == "UL By:" && substr($field,0,12) != "UL By:UL By:") {
					$field = substr($field,6,strlen($field)-6);
					echo "<td align='center' nowrap>* <a href='http://www.meedios.com/forum/profile.php?mode=viewprofile&u=" . $field . "'>$field</a> *</td>";
				}
				else echo "<td align='center' nowrap><a href='http://www.meedios.com/forum/profile.php?mode=viewprofile&u=" . $field . "'>$field</a></td>";
			}
...
function change_flag($file, $id, $version, $state, $status) {
	...
	if ($file == "ALL") {
		...
		echo "<br>Click <a href='admin.php'>here</a> to return to admin panel.<br>\n";
		echo "Click <a href='process.php?mode=manage_flagged'>here</a> to continue managing flagged.<br>\n";
	}
...
function removePluginFile($file) {
	...
	//ADD DELETE: DELETE FROM PLUGINS DIRECTORY TOO!!!!
				...
				echo "Deleting file and DB entries...$file <br>";
