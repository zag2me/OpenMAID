...
{	
	...
	$sql = "SELECT plugin_ID, plugin_Name, plugin_Version, plugin_State, plugin_Date, plugin_Author, profil_id, plugin_ReviewFlag FROM plugins WHERE plugin_ReviewFlag = 'TRUE'";
	$res = mysql_query($sql);
...
function UpdateCurrentPlugin($pluginID)
{
	...
	if ($version == -1)
	{
		...
		if ($version == -1)
		{
			...
			if ($version == -1)
			{
				echo("Error finding current version of plugin.<br>");
			}
		}
	}
...
//Only the most recently uploaded plugin will be returned
function GetMostRecentPluginObject($pluginID)
{
	ConnectOnce();
	
		$sql = "SELECT plugin_ID, plugin_Name, plugin_State, plugin_Version, plugin_DownloadSize, plugin_VersionValue FROM plugins WHERE plugin_ID = '$pluginID' and plugin_ReviewFlag = 'FALSE' GROUP BY plugin_VersionValue ORDER BY plugin_VersionValue DESC LIMIT 1";
		$res = mysql_query($sql);
		return mysql_fetch_object($res);
}
...
function IncrementDownloadCount($pluginID, $pluginVersion, $user)
{
	...
	//Check for ip address repeatedly downloading same plugin
	//select last download of current plugin from the users ip address
	$sql = "SELECT download_Date FROM userdownloads WHERE plugin_ID = '$pluginID' and plugin_Version = '$pluginVersion' and download_ip = '" . $_SERVER['REMOTE_ADDR'] . "' ORDER BY download_Date DESC LIMIT 1";
	$ip_result = mysql_query($sql);
	
	//check download dates of the ip address against list
	//if ip address has made this dl request more than once in 5 seconds, exit
	//if ip address is unique or not excessive, continue on
	$now = time();
	$enreg = mysql_fetch_array($ip_result);
	
	//echo "now: $now <br> ip address: " . $_SERVER['REMOTE_ADDR'] . "<br>";
	//echo "referrer: " . $_SERVER['HTTP_REFERER'] . "<br> request uri: " . $_SERVER['REQUEST_URI'] . "<br>";
	
	if ( ($now - strtotime($enreg['download_Date'])) < 5) {
		$wait = 5 - ($now - strtotime($enreg['download_Date']));
		echo "You can't download the same plugin twice in less than 5 seconds.<br>";
		echo "Please be kind to our server and wait $wait seconds before trying again.<br><p>";
		echo "If you feel you've reached this page in error or if there appears to be a problem, ";
		echo "please report it in the <a href='http://www.meedios.com/forum/viewtopic.php?t=947&highlight='>OpenMAID Web Site Support</a> forum thread.";
		exit(255);
	}
	$ip_time_diff = $now - strtotime($enreg['download_Date']);
	$ip_result = QuerySingleValue($sql);
	
	//echo "ip_time_diff: $ip_time_diff <br> ip_result: $ip_result <br>";
	...
	$user_result = QuerySingleValue($sql);

	//if user record is found, this is a dupe download request.
	if ($user_result == -1 || $user == 'Guest') $is_dupe = "false";
	else $is_dupe = "true";

	//if dl request from same ip more than once per 30 seconds, do nothing.
	//otherwise, log the download and increment the download counters
	if ($ip_time_diff > 30 || $ip_result == -1)
	{
		$sql = "INSERT INTO userdownloads (plugin_ID, plugin_Version, download_Date, download_ip, download_profil_id, is_dupe) VALUES ('$pluginID','$pluginVersion',NOW(), '" . $_SERVER['REMOTE_ADDR'] . "', '$user', '$is_dupe')";
		mysql_query($sql);
		
		//get aggregate count for this version/state of the plugin
		$sql = "SELECT COUNT(download_profil_id) FROM userdownloads WHERE plugin_ID = '$pluginID' and plugin_Version = '$pluginVersion' and is_dupe = 'false'";	
		$thisversion_downloads = QuerySingleValue($sql, 0);
		//get aggregate count for all versions of this plugin
		$sql = "SELECT COUNT(download_profil_id) FROM userdownloads WHERE plugin_ID = '$pluginID' and is_dupe = 'false'";	
...
{
	ConnectOnce();
	$sql = "SELECT COUNT(download_profil_id) FROM userdownloads WHERE plugin_ID = '$pluginID' and download_Date = NOW() and is_dupe='false'";	
...
{
	ConnectOnce();
	$sql = "SELECT COUNT(download_profil_id) FROM userdownloads WHERE plugin_ID = '$pluginID' and download_Date > DATE_SUB(CURDATE(),INTERVAL 30 DAY) and is_dupe='false'";	
	return QuerySingleValue($sql, 0);	
}
...
{
	ConnectOnce();
	$sql = "SELECT COUNT(download_profil_id) FROM userdownloads WHERE plugin_ID = '$pluginID' and plugin_Version = '$pluginVersion' and download_Date = NOW() and is_dupe='false'";	
	return QuerySingleValue($sql, 0);
}
...
{
	ConnectOnce();
	$sql = "SELECT COUNT(download_profil_id) FROM userdownloads WHERE plugin_ID = '$pluginID' and plugin_Version = '$pluginVersion' and download_Date > DATE_SUB(CURDATE(),INTERVAL 30 DAY) and is_dupe='false'";	
	return QuerySingleValue($sql, 0);	
}
...
function InitializeDatabase()
{
	...
	LogCreateTable("Plugins","CREATE TABLE IF NOT EXISTS `plugins` (
					  ...
					  INDEX KEY `profile_id` (`profil_id`),
					  INDEX KEY `author` (`plugin_Author`)
	...
	LogCreateTable("Images","CREATE TABLE IF NOT EXISTS `images` (
					  ...
					  INDEX KEY `id_version` (`plugin_ID`,`plugin_Version`)
	...
	LogCreateTable("User Downloads","CREATE TABLE IF NOT EXISTS `userdownloads` (
					  ...
					  `download_ip` varchar(15) collate latin1_general_ci NOT NULL default '0.0.0.0',
					  `download_profil_id` tinytext collate latin1_general_ci NOT NULL,
					  `is_dupe` enum('true','false') collate latin1_general_ci NOT NULL default 'false',
					  INDEX KEY `id_version` (`plugin_ID`,`plugin_Version`),
					  INDEX KEY `date` (`download_Date`)
					) ENGINE=MyISAM DEFAULT CHARSET=latin1 COLLATE=latin1_general_ci;");
