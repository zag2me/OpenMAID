...
function EditForm($plugin){	
	...
	printTextBox("Author","plugin_Author", $plugin->plugin_Author);
	printTextBox("Author Email","plugin_AuthorEmail", $plugin->plugin_AuthorEmail);
	printTextBox("Web Site Link","plugin_Site", $plugin->plugin_Site);
	printTextBox("Direct Download Link","plugin_DirectDownloadURL", $plugin->plugin_DirectDownloadURL);
...
if ( (IsAdmin($currentUser)) || (!IsAdmin($currentUser) && $currentUser == $plugin->plugin_Author) ) echo"";
else slowdie( "Sorry, you are not authorized to edit this addon.");
	...
	$plugin_AuthorEmail = $_POST["plugin_AuthorEmail"];
	$plugin_Site = $_POST["plugin_Site"];
	$plugin_DirectDownloadURL = $_POST["plugin_DirectDownloadURL"];
	...
	mysql_query($sql);
	$sql = "UPDATE plugins SET plugin_Author = '$plugin_Author', plugin_AuthorEmail = '$plugin_AuthorEmail', plugin_Site = '$plugin_Site', plugin_DirectDownloadURL = '$plugin_DirectDownloadURL' WHERE plugin_ID = '$plugin_ID' AND plugin_Version = '$plugin_Version' AND plugin_State = '$plugin_State'";
	mysql_query($sql);
...
if ( (IsAdmin($currentUser)) || (!IsAdmin($currentUser) && $currentUser == $plugin->plugin_Author) ) EditForm($plugin);
else slowdie( "Sorry, you are not authorized to edit this addon.");
