...
function printToolHome($user)
{
	...
	$sql = "SELECT plugin_Name, plugin_Author, profil_id, plugin_ID, count(*) AS plugin_Count FROM plugins WHERE profil_id != '' AND plugin_ReviewFlag != 'TRUE' AND plugin_current = true GROUP BY profil_id ORDER BY plugin_Count DESC LIMIT 10
";
	$res = mysql_query($sql);
	echo "<br><br><b>Top 10 Plugin Contributors</b><br><table>";
	while (($enreg=@mysql_fetch_array($res))) {
		echo "<tr><td> [" . $enreg["plugin_Count"] . " plugins] </td><td>" . $enreg["plugin_Author"] . " (<a href=\"thelist.php?profile_id=" . urlencode($enreg["profil_id"]) . "\"><i>" . $enreg["profil_id"] . "</i></a>) </td></tr>";
	}
	echo "</table>";
