...
$mode = $_GET["mode"];
if ($mode != "managedb" && $mode != "fixmppnames") echo '<div id="leftbar">';
...
function printAdminHome($user)
{
	...
	//check if we're down for maintenance
	...
	if ($semaphore) printLine ("<b>OpenMAID is currently down for maintenance! <a href='admin.php?mode=maint'>Change Status</a></b><br>");
...
function printSyncCode()
{
	...
	$source_file = "openmaid_source.gzip";
	$source_url = "http://beta.openmaid.meedios.com/openmaid_source.gzip";
...
function printClearMAIDCache()
{
	...
	delfile($all_cache);
	delfile($all_cache.".temp");
	echo "$all_cache is deleted...<br>";
	delfile($extended_cache);
	delfile($extended_cache.".temp");
	echo "$extended_cache is deleted...<br>";
	delfile($summary_cache);
	delfile($summary_cache.".temp");
	echo "$summary_cache is deleted...<br>";
	delfile($sync_cache);
	delfile($sync_cache.".temp");
	echo "$sync_cache is deleted...<br><br>";
	
	echo "Done.<br />\n";
}

function printRegenDLCounts() {
	//loop through new OpenMAID DB and set download counts for each plugin based on downloads logged in the userdownloads table
...
function printMaint()
{
	printSectionTitle("Down For Maintenance");
	$mode = $_SERVER['SCRIPT_NAME'] . "?mode=maint";
	$request = "";
	$request = $_POST["DownForMaint"];
	$post_reason = "";
	$post_reason = stripslashes($_POST["reason"]);
	$file_reason = "";
	if (file_exists('maint.now')) {
		$semaphore = true;
		$fh = fopen('maint.now', 'r');
	...
	if (($semaphore == true || $request == "down") && $request != "up") $status = "Down For Maintenance";
	elseif ($semaphore == false || $request == "up") $status = "Online";
	printH2("OpenMAID is currently: $status");
	
	//if semaphore file doesn't exist and hasn't been requested, display form asking for reason and a button to submit and put the site in an offline mode
	if (!$request && $semaphore == false)
	{
		printPara("Use this page to take OpenMAID offline for users if you need to perform any extensive maintenance.  Admins will still have full use of the OpenMAID site, but users will no longer be able to browse, upload, or download while the site is in the 'Down for Maintenance' mode.  Remember to come back to this page to put the site back online.");
		printPara("Fill in a reason if you want one displayed to the web site visitors and then click the Down For Maintenance button to take OpenMAID offline for maintenance.");
		echo("<form method='post' action='$mode'><table><tr><td align='right'>Reason: </td><td><textarea name='reason' cols='40' rows='5'></textarea></td></tr>");
		echo("<tr><td>&nbsp;</td><td><input type='hidden' name='DownForMaint' value='down' /> <input type='submit' value='Down For Maintenance'/></td></tr></table></form>");
	}

	//if semaphore file doesn't exist and has been requested, create file, print reason on screen and acknowledge that site is now 'Down for Maintenance'
	elseif ($request == "down" && $semaphore == false)
	{
		echo "Going down for maintenance...<br>";
		$fh = fopen('maint.now', 'w') or die("can't open file");
		fwrite($fh, $post_reason);
		fclose($fh);		
		if ($post_reason != "") echo "Reason: <br> $post_reason<br><br>";
		else echo "Reason: <i> (no reason given) </i> <br><br>";
		echo "<b><a href='$mode'>Back</a> &nbsp;&nbsp;|&nbsp;&nbsp; <a href='admin.php'>Admin Home</a></b><br>";
	}

	//if semaphore file exists and maintenance has been requested, change file, print new reason on screen and acknowledge that site is now 'Down for Maintenance'
	elseif ($request == "down" && $semaphore == true)
	{
		echo "Changing maintenance reason...<br>";
		if ($post_reason != "") echo "<br>New Reason:<br><br> $post_reason <br><br>";
		else echo "Reason: <i> (no reason given) </i> <br><br>";
		$fh = fopen('maint.now', 'w') or die("can't open file");
		fwrite($fh, $post_reason);
		fclose($fh);		
	...
	//if semaphore file exists and no request to exit 'down for maintenance' mode, display 'down for maintenance' mode and the reason (if given in semaphore file) and display link to put the site back online
	elseif (!$request && $semaphore == true)
	{
		printPara("Use this page to change the reason for maintenance and to exit maintenance mode and bring OpenMAID back online.  ");
		echo "We are down for maintenance...";
		if ($maint_reason != "" ) echo "<br>Reason:<br><br> $maint_reason <br><br>";
		else echo "<br>Reason: <i> (no reason given) </i> <br><br>";
		
		printPara("Fill in a new reason if you want one displayed to the web site visitors and then click the Change Reason button to change the reason for offline for maintenance.");
		echo("<form method='post' action='$mode'><table><tr><td align='right'>Change Reason: </td><td><textarea name='reason' cols='40' rows='5'></textarea></td></tr>");
		echo("<tr><td>&nbsp;</td><td><input type='hidden' name='DownForMaint' value='down' /> <input type='submit' value='Change Reason'/></td></tr></form>");

		echo("<form method='post' action='$mode'>");
		echo("<tr><td>&nbsp;</td><td><input type='hidden' name='DownForMaint' value='up' /> <input type='submit' value='Exit Maintenance Mode'/></td></tr></table></form>");

		echo "<b><a href='$mode'>Back</a> &nbsp;&nbsp;|&nbsp;&nbsp; <a href='admin.php'>Admin Home</a></b><br>";
	}

	//if semaphore file exists and request to put site back online exists, remove semaphore file and acknowledge the site is now back online
	elseif ($request == "up" && $semaphore == true)
	{
		unlink('maint.now');
		echo "We are back online...<br><br>";
		echo "<b><a href='$mode'>Back</a> &nbsp;&nbsp;|&nbsp;&nbsp; <a href='admin.php'>Admin Home</a></b><br>";
	}

	//if semaphore file doesn't exist and request to put site back online exists, acknowledge the site is already online
	elseif ($request == "up" && $semaphore == false)
	{
		echo "We are already online...<br>";
		echo "<b><a href='$mode'>Back</a> &nbsp;&nbsp;|&nbsp;&nbsp; <a href='admin.php'>Admin Home</a></b><br>";
	}
...
if ($mode != "managedb"){
	...
	echo '<a href="?mode=home">Admin Home</a><br />';
	echo '<a href="?mode=maint">Down for Maintenance</a><br />';
	...
	echo '<a href="?mode=managedb">Manage Database</a><br />';
	echo '<a href="?mode=dbinfo">Database Information</a><br />';
	echo '<a href="?mode=phpinfo">PHP Information</a><br />';
	echo '<a href="?mode=initdb">Initialize Database</a><br />';
	echo '<a href="?mode=initdir">Initialize Directories</a><br />';
	echo '<a href="?mode=check">Check for Problems</a><br />';
	echo '<a href="?mode=clearcache">Clear MAID Cache Files</a><br />';
	echo '<a href="?mode=regendlcounts">Regen DL Counts</a><br />';
	echo '<a href="http://beta.openmaid.meedios.com/admin.php?mode=sourcecode" target="_blank">Archive Source Code</a><br />';
	echo '<a href="?mode=synccode">Sync Source Code</a><br />';
	echo '<a href="http://beta.openmaid.meedios.com/openmaid_source.gzip">Download Source Code</a><br />';
	echo '</div>';
	}
